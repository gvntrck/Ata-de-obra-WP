...
<body>
    ...
    <script>
        ...
        function adicionarItem(tipo) {
            ...
            if (!valor) {
                Swal.fire('Atenção', 'Por favor, preencha o campo', 'warning');
                return;
            }
        ...
        function salvarItem(id, tipo) {
            ...
            if (!valor) {
                Swal.fire('Atenção', 'O campo não pode estar vazio', 'warning');
                return;
            }
            ...
            postAction({
                ...
                Swal.fire('Erro', data.message || 'Erro ao editar', 'error');
            }).catch(() => {
        ...
        function deletarAta(ataId) {
            Swal.fire({
                title: 'Tem certeza?',
                text: 'Deseja deletar esta ata? Esta ação não pode ser desfeita!',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                ...
                if (result.isConfirmed) {
                    postAction({
                        ...
                        if (data.success) {
                            Swal.fire('Sucesso!', 'Ata deletada com sucesso!', 'success');
                            carregarRelatorios(1);
                            return;
                        }

                        Swal.fire('Erro', data.message || 'Erro ao deletar ata', 'error');
                    }).catch(() => {
                        Swal.fire('Erro', 'Erro ao deletar ata. Tente novamente.', 'error');
                    });
                }
            });
        }
